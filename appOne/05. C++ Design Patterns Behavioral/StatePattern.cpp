...
class PurschasedState : public State {
    State* nextState = nullptr;
public:
    PurschasedState(State* nextState) : nextState(nextState) {};
...
public:
    ...
    std::string getDescription() override {
        return "Current State: INTRANSIST - your item is on the way\n";
    }
    State* getNextState() override { return nextState; }
};
...
public:
    ...
    std::string getDescription() override {
        return "Current State: DELIVERED - Your item is delivered\n";
    }
    State* getNextState() override { return nextState; }
};
...
int main(int argc, const char * argv[]) {
    ...
    InTransitState* inTransisState = new InTransitState(deliveredState);
    PurschasedState* purchaseState = new PurschasedState(inTransisState);

    Purchase* purchase = new Purchase("Shoes", purchaseState);
