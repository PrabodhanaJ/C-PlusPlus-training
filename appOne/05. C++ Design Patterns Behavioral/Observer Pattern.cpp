...
public:
    ...
    void unsubscribe(Subscriber* subscriber) override{
        subscribers.erase(std::remove_if(subscribers.begin(),
            ...
            { return s->getName() == subscriber->getName();  }), subscribers.end());
    }
    void publish(const std::string& message) override {
        for (auto subcriber : subscribers) {
            subcriber->notify(groupName, message);
        }
    }
...
int main(int argc, const char * argv[]) {
    ...
    ChatUser* user2 = new ChatUser("anne");
    ChatUser* user3 = new ChatUser("harry");
    ...
    group1->publish("Special sale on garding supplies!");
    group2->publish("It's national dog day everone");
